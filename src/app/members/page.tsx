...
import { getGroupedMembers, type CouncilMember } from "@/data/members";
...
export default function MembersPage() {
  ...
  const activeCount = (list: CouncilMember[]) =>
    list.filter((m) => !m.resigned).length;
  const total = Array.from(grouped.values()).reduce(
    (sum, list) => sum + activeCount(list),
    0,
  );
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {list.map((member, i) => (
                <Reveal key={member.id} delay={(i % 6) * 60}>
                  <div
                    className={`flex items-center gap-4 border border-border-soft bg-paper-alt p-5 ${
                      member.resigned ? "opacity-60" : ""
                    }`}
                  >
                    ...
                    <div>
                      <h3 className="font-serif text-base font-semibold text-ink">
                        <span className={member.resigned ? "line-through" : ""}>
                          {member.name}
                        </span>
                        {member.resigned && (
