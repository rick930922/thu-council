    ...
    <div>
      ...
      <section className="mx-auto max-w-6xl px-6 py-16">
        <div className="flex items-baseline justify-between mb-8">
          <h2 className="font-serif text-2xl font-bold text-ink">最新消息</h2>
          <Link
            href="/news"
        ...
        <div className="grid gap-8 sm:grid-cols-3">
          {latestNews.map((item) => (
            <Link
              key={item.slug}
              href={`/news/${item.slug}`}
              className="group flex flex-col border border-border-soft bg-paper-alt p-6 hover:border-wine/50 transition-colors"
            >
          ...
          <div className="grid gap-px bg-border-soft sm:grid-cols-2 lg:grid-cols-4 border border-border-soft">
            {quickLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="group flex flex-col justify-between bg-paper p-7 hover:bg-paper-alt transition-colors"
              >
